                ...
                if (sbdKeys.includes(docKey)) {
                    ...
                    if (!userSbd) {
                        ...
                        checks.push({
                            ...
                            section: 'SBD 6.1',
                            requirementName: 'Declared',
                            status: 'fail',
                            message: '',
                            yourData: 'Not captured',
                            docType: docKey
                        });
                        checks.push({
                        ...
                        const prefPoints = metadata.claiming_points || metadata.claimingPoints || metadata.preference_points_claimed || metadata.preferencePointsClaimed || metadata.preference_points_claim || metadata.preferencePointsClaim || metadata.points_claimed || metadata.pointsClaimed || metadata.preference_points || metadata.preferencePoints || metadata.claimed_points || metadata.claimedPoints || metadata.preference_score_claimed || metadata.preferenceScoreClaimed || metadata.preference_points_score || metadata.preferencePointsScore || metadata.points || metadata.total_points_claimed || metadata.totalPointsClaimed || metadata.bbbee_points_claimed || metadata.bbbeePointsClaimed || metadata.b_bbee_points_claimed || metadata.specific_goals_points_claimed || metadata.specificGoalsPointsClaimed || metadata?.specific_goals?.points_claimed || metadata?.specificGoals?.pointsClaimed || metadata?.preference?.points_claimed || metadata?.preference?.pointsClaimed;
