-- MASTER SYNC V2: Frontend Contract Alignment & Security Hardening
-- Generated at: 2026-02-07
-- Description: Standardizes names (camelCase RPCs), fixes drift (company_docs -> compliance_docs), and locks down security.

BEGIN;

-- 0. PRE-FLIGHT: Drop Dependent Views (Fixes 'cannot drop columns from view' error)
DROP VIEW IF EXISTS public.view_compliance_summary;

-- =====================================================================
-- 1. TABLE & COLUMN STANDARDIZATION (Option A: compliance_documents)
-- =====================================================================

DO $$
BEGIN
    -- Pre-Migration: Ensure compliance_documents has necessary columns if it already exists
    -- (This fixes the 'column is_verified does not exist' error during INSERT)
    ...
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'company_documents') THEN
        IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'compliance_documents') THEN
            ...
            SELECT 
                id, profile_id, doc_type::text, file_path, expiry_date, is_verified, created_at, updated_at
            FROM public.company_documents
            ON CONFLICT (id) DO NOTHING; -- Avoid dups
...
ALTER TABLE public.compliance_documents ADD COLUMN IF NOT EXISTS status text DEFAULT 'missing'; 
...
-- Public read is REMOVED (was "Public read for admins table") to satisfy strict security requirements.
-- This means `is_admin_check()` helpers must use SECURITY DEFINER or query this table via RLS.

-- =====================================================================
-- 3. RLS POLICIES (Least Privilege)
-- =====================================================================

-- Helper macro for "Is Admin" check in policies:
...
DROP POLICY IF EXISTS "cd_update_own" ON public.compliance_documents;
CREATE POLICY "cd_update_own" ON public.compliance_documents FOR UPDATE USING (
    user_id = auth.uid()
);
...
SELECT 
    id,
    user_id, -- Canonical name
    user_id as profile_id, -- Backward compat alias if needed
    doc_type,
    status,
    expiry_date,
    title,
...
-- 5.1 get_admin_dashboard_snapshot (Already JSON, ensuring camelCase)
...
-- 5.2 get_admin_analytics (Convert to camelCase keys)
...
-- 5.3 get_admin_users (Convert to JSON Array with camelCase keys)
...
CREATE INDEX IF NOT EXISTS idx_sub_history_composite ON public.subscription_history(user_id, created_at, status);
