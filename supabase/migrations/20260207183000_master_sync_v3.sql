-- MASTER SYNC V3: Frontend Contract Alignment, Security Hardening & Compatibility
-- Generated at: 2026-02-07
-- Description: Standardizes names, fixes drift, enforces Admin Security, and provides backward compat views.

BEGIN;

-- 0. PRE-FLIGHT: Drop Dependent Views to allow column alterations
DROP VIEW IF EXISTS public.view_compliance_summary;
DROP VIEW IF EXISTS public.company_documents; -- Drop compat view if it exists from prev run
...
ALTER TABLE public.compliance_documents ADD COLUMN IF NOT EXISTS doc_type text; -- Ensure doc_type exists
...
CREATE POLICY "cd_update_own_or_admin" ON public.compliance_documents FOR UPDATE USING (
    user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.admins WHERE id = auth.uid())
);
