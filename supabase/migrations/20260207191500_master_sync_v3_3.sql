-- MASTER SYNC V3.3: Frontend Contract Alignment, Security Hardening & Compatibility
-- Generated at: 2026-02-07
-- Description: Standardizes names (v3.3 Patch: Fix view dependency error by safe drop)
...
-- Safe Drop: Only drop company_documents IF it is a VIEW (not a legacy table)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 
        FROM pg_views 
        WHERE schemaname = 'public' AND viewname = 'company_documents'
    ) THEN
