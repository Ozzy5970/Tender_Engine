-- MASTER SYNC V3.5: Frontend Contract Alignment, Security Hardening & Compatibility
-- Generated at: 2026-02-07
-- Description: Standardizes names (v3.5 Patch: Preflight doc_type validation)
...
DO $$
BEGIN
  -- drop company_documents ONLY if it's a view
  IF EXISTS (
    SELECT 1
    FROM pg_class c
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE n.nspname = 'public'
      AND c.relname = 'company_documents'
      AND c.relkind = 'v'  -- view
  ) THEN
    EXECUTE 'DROP VIEW public.company_documents';
  END IF;

  -- drop compliance summary view if it exists
  IF EXISTS (
    SELECT 1
    FROM pg_class c
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE n.nspname = 'public'
      AND c.relname = 'view_compliance_summary'
      AND c.relkind = 'v'
  ) THEN
    EXECUTE 'DROP VIEW public.view_compliance_summary';
  END IF;
END $$;
...
BEGIN
    ...
    IF EXISTS (
        ...
        -- 1. Preflight: Sanitize invalid doc_types (Set to NULL if not in enum)
        -- We cast enum_range to text array and check against it
        UPDATE public.compliance_documents
        SET doc_type = NULL
        WHERE doc_type IS NOT NULL 
        AND doc_type NOT IN (
            SELECT unnest(enum_range(null::public.company_doc_type))::text
        );
...
SELECT 
    ...
    udt_name 
FROM information_schema.columns 
WHERE table_name = 'compliance_documents' AND column_name = 'doc_type';
